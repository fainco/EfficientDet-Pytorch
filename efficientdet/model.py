...
class BiFPN(nn.Module):
    ...
    def __init__(self, num_channels, conv_channels, first_time=False, epsilon=1e-4, onnx_export=False, attention=True):
        ...
        # Weight,这里有2有3，在之后的融合特征的时候会用到这个权重。这个权重也会根据梯度更新。
    ...
    def forward(self, inputs):
        """
        特征其实就是按照下面这张图进行组合的，组合的方式按照权重比例融合(attendtion)，也可以等比例融合。
...
class Regressor(nn.Module):
    ...
    def forward(self, inputs):
        """
            inputs = (p3,p4,p5)
            p = (c,w,h)
            循环读入每层feature，保存经过进一系列操作后的feature。len(feats)==len(feat)
        """
        feats = []
        for feat, bn_list in zip(inputs, self.bn_list): 
            ...
            feat = feat.contiguous().view(feat.shape[0], -1, 4) # view方法要求Tensor是连续的
...
class Classifier(nn.Module):
    """
    modified by Zylo117
    和Regressor基本做法一样，只是最终通过sigmoid得到分类结果。
    """
