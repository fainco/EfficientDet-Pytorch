def _cpu_soft_nms(dets, sc, Nt=0.3, sigma=0.5, thresh=0.001, method=2):
    """
    py_cpu_softnms
...
def soft_nms(dets, sigma=0.5, Nt=0.3, threshold=0.001, method=1):
    keep = _cpu_soft_nms(np.ascontiguousarray(dets, dtype=np.float32),
                        np.float32(sigma), 
                        np.float32(Nt),
...
def nms_test():
    boxes = np.array([[200, 200, 400, 400], [220, 220, 420, 420], [200, 240, 400, 440], [240, 200, 440, 400], [100, 100, 20, 20]], dtype=np.float32)
    ...
    def plot_img(boxes,score):
        ...
        cv2.resizeWindow('image',(512,521));
