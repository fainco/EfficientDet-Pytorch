...
def round_filters(filters, global_params):
    ...
    filters *= multiplier # 增加这个比例的通道数
    min_depth = min_depth or divisor # 设置最低通道数
    new_filters = max(min_depth, int(filters + divisor / 2) // divisor * divisor) # (16+8/2) =20//8=2 *8=16
    if new_filters < 0.9 * filters:  # prevent rounding by more than 10%
        new_filters += divisor  # 舍去超过10%的数值，则增加一个divisor的数值
...
def round_repeats(repeats, global_params):
    """ 
        ...
        根据深度因子修改卷积深度repeats的数值。比如卷积深度计划是9层，
        乘以深度因子后1.1，变为9.9。因为卷积层数只可能是整数，所以上采样到10层。
    """
...
def drop_connect(inputs, p, training):
    """ 
        Drop connect. 
        直白话就是，随机屏蔽一些特征，让模型用剩下的特征计算。
        因为特征减少，所以，对没有屏蔽的特征也做了放大处理。
    """
...
def get_same_padding_conv2d(image_size=None):
    ...
    if image_size is None:
        ...
        # python的偏函数，此时的partial，即带有已知参数的Conv2dStaticSamePadding。
        # 这个image_size参数没用到。 并且这两个方法只是写法不同，没有区别。
        # 具体写法了解可以查阅nn.Conv2d与nn.F.conv2d
